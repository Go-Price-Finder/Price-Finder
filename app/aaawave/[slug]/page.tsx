...
export async function generateMetadata({
  ...
  const product = await getRealProduct("aaawave", slug);
  if (!product) return { title: "Not found — Go Price Finder" };
  return {
    // Suffix disambiguates same-named SKUs — see getProductTitleSuffix's
    // own comment. AAAwave's feed has real same-name variants (pack sizes,
    // capacity tiers), so the suffix is load-bearing here.
    title: `${product.name} — ${await getProductTitleSuffix(product)} — AAAwave — Go Price Finder`,
    description: product.description.slice(0, 155),
  };
}

export default async function AaawaveProductPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const related = (await getAllRealProducts())
    .filter((p) => p.partnerId === "aaawave" && p.category === product.category && p.slug !== product.slug)
    .slice(0, 4);
  ...
  return (
    <>
      ...
      <main className="flex-1">
        {/* Breadcrumb — Home / AAAwave / Product Name */}
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="mt-2 flex flex-col gap-2 sm:flex-row">
                {isDiscontinuedAtRetailer(product.id) ? (
                  <div className="flex flex-1 items-center justify-center rounded-full border border-gilt-500/30 bg-noir-800 px-6 py-3 text-sm font-medium text-ivory-300">
                    AAAwave no longer stocks this item
                  </div>
                ) : (
                  <a
              ...
              {!isDiscontinuedAtRetailer(product.id) && (
                <p className="text-xs text-ivory-400">
                  &ldquo;View on AAAwave&rdquo; takes you to AAAwave&rsquo;s own store to complete your purchase.
                </p>
              )}
