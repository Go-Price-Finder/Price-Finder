...
export async function generateMetadata({
  ...
  return {
    ...
    title: `${product.name} — ${await getProductTitleSuffix(product)} — Canvas Vows — Go Price Finder`,
    description: product.description.slice(0, 155),
  };
}

export default async function CanvasVowsProductPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  // Not filtered by category — every Canvas Vows product shares the same
  // raw "Uncategorized" value (see app/canvas-vows/page.tsx), so this is
  // effectively "other Canvas Vows products" already.
  const related = (await getAllRealProducts())
    .filter((p) => p.partnerId === "canvas-vows" && p.slug !== product.slug)
    .slice(0, 4);
  ...
  return (
    <>
      ...
      <main className="flex-1">
        {/* Breadcrumb — Home / Canvas Vows / Product Name */}
          ...
          <div className="grid gap-10 lg:grid-cols-2">
            ...
            <div className="flex flex-col gap-4">
              ...
              <p className="text-xs text-ivory-400">
                &ldquo;View on Canvas Vows&rdquo; takes you to Canvas Vows&rsquo; own store to complete your purchase.
              </p>
